...
export class ShortExpiryReportComponent implements OnInit {
  ...
  // get endDate() {
  //   return this.shortExpiryForm.controls['endDate'].value;
  // }
  ...
  //   } else {
  //     this.shortExpiryForm.patchValue({
  //       endDate: null
  //     })
  //   }
  // }
  ...
  searchReport() {
    let startDate: Date = new Date(this.shortExpiryForm.value.startDate);
    ...
    console.log("Data form value...", JSON.stringify(this.shortExpiryForm.value));
    let reqObjForShortExpiryReport = {
    ...
    console.log("Data form data", JSON.stringify(reqObjForShortExpiryReport, null, 4));

    this.inventoryService.getShortExpiryReports(reqObjForShortExpiryReport).subscribe((response) => {
  ...
  exportToxlsx(criteria: any) {
    if (this.shortExpiryList.length > 0) {
      let array = this.shortExpiryList.filter(function (obj) {
        for (var key in obj) {
          if (obj[key] == null) {
            obj[key] = "";
          }
      ...
      if (array.length != 0) {
        ...
        let wb_name = "Short Expiry Report";
        const criteria_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(criteria);
        const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.shortExpiryList, { header: (head) });
      ...
      this.confirmationService.alert(this.currentLanguageSet.inventory.shortExpiryReportdownloaded, 'success');
    } else {
