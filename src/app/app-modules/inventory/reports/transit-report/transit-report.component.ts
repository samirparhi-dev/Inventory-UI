...
export class TransitReportComponent implements OnInit {
  ...
  searchReport() {
    let startDate: Date = new Date(this.transitForm.value.startDate);
    let endDate: Date = new Date(this.transitForm.value.endDate);
    ...
    console.log("Data form data", JSON.stringify(reqObjForTransitReport, null, 4));

    this.inventoryService.getTransitReports(reqObjForTransitReport).subscribe((response) => {
  ...
  exportToxlsx(criteria: any) {
    if (this.transitList.length > 0) {
      let array = this.transitList.filter(function (obj) {
        for (var key in obj) {
          if (obj[key] == null) {
            obj[key] = "";
          }
      ...
      if (array.length != 0) {
        ...
        let wb_name = "Transit Report";
        const criteria_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(criteria);
        const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.transitList, { header: (head) });
      ...
      this.confirmationService.alert(this.currentLanguageSet.inventory.transitReportdownloaded, 'success');
    } else {
