...
export class ExpiryReportComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
    ...
    searchReport() {
      let startDate: Date = new Date(this.expiryForm.value.startDate);
      let endDate: Date = new Date(this.expiryForm.value.endDate);
      ...
      console.log("Data form data", JSON.stringify(reqObjForExpiryReport, null, 4));
  
      this.inventoryService.getExpiryReports(reqObjForExpiryReport).subscribe((response) => {
    ...
    exportToxlsx(criteria: any) {
      if (this.expiryList.length > 0) {
        let array = this.expiryList.filter(function (obj) {
          for (var key in obj) {
            if (obj[key] == null) {
              obj[key] = "";
            }
        ...
        if (array.length != 0) {
          ...
          let wb_name = "Expiry Report";
          const criteria_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(criteria);
          const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(this.expiryList, { header: (head) });
        ...
        this.confirmationService.alert(this.currentLanguageSet.inventory.expiryReportdownloaded, 'success');
      } else {
