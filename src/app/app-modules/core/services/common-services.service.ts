...
export class CommonService {
    ...
    getRegistrationData() {
      return this.http.post(environment.getBeneficiaryDataURL, '').map(res => res.json())
    }
    getStateDistricts(stateID) {
      return this.http.get(`${environment.getStateDistrictsURL}${stateID}`).map(res => res.json());
    }

    searchBeneficiary(dataObj) {
      return this.http.post(environment.searchBeneficiaryURL, dataObj).map(res => res.json());
    }
}
